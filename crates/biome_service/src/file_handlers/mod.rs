...
impl DocumentFileSource {
    ...
    fn try_from_path(path: &Path) -> Result<Self, FileSourceError> {
        ...
        let extension = &match path {
            _ if path_bytes[path_bytes.len() - 5..].eq_ignore_ascii_case(b".d.ts") => {
                Some(Cow::Borrowed("d.ts".as_ref()))
            }
            _ if path_bytes[path_bytes.len() - 6..].eq_ignore_ascii_case(b".d.mts") => {
                Some(Cow::Borrowed("d.mts".as_ref()))
            }
            _ if path_bytes[path_bytes.len() - 6..].eq_ignore_ascii_case(b".d.cts") => {
                Some(Cow::Borrowed("d.cts".as_ref()))
            }
            path => {
                // We assume the file extensions are case-insensitive
                // and we use the lowercase form of them for pattern matching
                path.extension()
                    .map(|extension| extension.to_ascii_lowercase_cow())
            }
        }
